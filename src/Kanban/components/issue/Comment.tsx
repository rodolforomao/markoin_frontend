      ...
      <li className='mb-6 flex items-start gap-3'>
        ...
        <div className='grow'  >
          <div className='flex justify-between text-sm' >
            <span  className='font-semibold'>{username + (isMine ? ' (Você)' : '')}</span>
            <span style={{color:'var(--c-5)'}} className='ml-3 text-gray-700'>{parseDate(createdAt)}</span>
          </div>
          <span style={{color:'var(--c-5)'}} className='block text-gray-800'>{descr}</span>
          {isMine && (
            <button
              onClick={() => setOn(true)}
